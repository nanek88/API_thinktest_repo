...
@step('I want to login as user using details from {new_user_cache} into the system and save details in {logged_user_details_cache}')
def user_login(context, new_user_cache, logged_user_details_cache):
  try:
    ...
    if len(user) == 0:
      logging.info(f"User FAILED to Login!")
...
@Given('I want to delete contact with details in {contact_to_remove_cache}')
def delete_contact(context, contact_to_remove_cache):
    try:
        contact_details = cache.__getitem__(cacheObj, contact_to_remove_cache)
        if contact_details == []:
            return
        try:
...
def remove_contact(contact_id):
    ...
    try:
        url = f"{settings['Contact_List']['api_url']}{settings['Contact_Endpoints']['contacts']}/{contact_id}"
        response = getContactListEndpointResponse("delete", url, header)
        if response.status_code != 200 and response.text != 'Contact deleted':
            logging.info(f"Error deleting contact with id {contact_id}")
            raise Exception("Contact not deleted")
        else:
            logging.info(f"Contact with id: {contact_id} successfully deleted")
    except Exception as e:
        stopWorkflow(str(e))
